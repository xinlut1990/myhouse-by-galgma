...
{
public:
	stBoundingBox () { min[0] = min[1] = min[2] =1000;
	max[0] = max[1] = max[2] = -1000; 
	cubeTouchBottom = false;}
	stBoundingBox(const stBoundingBox& stbb){
		min[0] = stbb.min[0];
		min[1] = stbb.min[1];
		min[2] = stbb.min[2];
		max[0] = stbb.max[0];
		max[1] = stbb.max[1];
		max[2] = stbb.max[2];
		cubeTouchBottom = stbb.cubeTouchBottom;
	}
	double min[3];
	double max[3];
	bool cubeTouchBottom;
};
